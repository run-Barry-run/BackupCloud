...
class CaptionDataset(Dataset):
    def __init__(self) -> None:
        super().__init__()
        self.human_json = json.load(
            open('datasets/Eval/image/ChartQA/test_human.json')
        )
        self.aug_json = json.load(
            open('datasets/Eval/image/ChartQA/test_augmented.json')
        )
        self.img_dir = 'datasets/Eval/image/ChartQA/png'
        self.first_len = len(self.human_json)
        self.second_len = len(self.aug_json)
    ...
    def __getitem__(self, index):
        if index < self.first_len:
            data = self.human_json[index]
        else:
            data = self.aug_json[index-self.first_len]
        img_path = os.path.join(self.img_dir, data['imgname'])
