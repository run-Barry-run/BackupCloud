...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], prompt=None):
        ...
        if self.k_cache is None or self.v_cache is None:
            ...
            keys = self.k_cache[:bsz, :start_pos + seqlen].bfloat16()
            values = self.v_cache[:bsz, :start_pos + seqlen].bfloat16()

        xq = xq.bfloat16().cuda()
        keys = keys.bfloat16().cuda()
        values = values.bfloat16().cuda()

        output = flash_attn_func(
            xq, keys, values, dropout_p=0.0, causal=mask is not None)
        output = output.contiguous().view(bsz, seqlen, -1)
        self.wo.to(output.device)
...
class TransformerBlock(nn.Module):
    ...
    def _forward_attention(self, x, start_pos, freqs_cis, mask, prompt):
        attn_result = self.attention.forward(self.attention_norm(x), start_pos, freqs_cis, mask, prompt)
        attn_result = attn_result.to(x.device)
...
class Transformer(nn.Module):
    ...
    def forward(self, examples, image=None, modal='image'):
        ...
        self.tok_embeddings = self.tok_embeddings.to(examples.device)
        ...
        if image is not None:
            ...
            image_tokens = self.encode_image(image, modal).to(h.device)
            self.end_tag[modal] = self.end_tag[modal].to(h.device)
            self.start_tag[modal] = self.start_tag[modal].to(h.device)
        ...
        self.layers.to(h.device)
        for layer in self.layers:
            layer.to(h.device)
            h = layer(h, start_pos, freqs_cis, mask).to(h.device)
        self.norm.to(h.device)
        h = self.norm(h)
        self.output.to(h.device)
